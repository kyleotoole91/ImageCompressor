...
{$ENDIF}
  ...
  public
    ...
    //Ocuco
    property jObject[const index: integer]: ISuperObject read GetO write PutO;
    property jBoolean[const index: integer]: boolean read GetB write PutB;
    property jInteger[const index: integer]: SuperInt read GetI write PutI;
    property jDouble[const index: integer]: Double read GetD write PutD;
    property jCurrency[const index: integer]: Currency read GetC write PutC;
    property jString[const index: integer]: SOString read GetS write PutS;
{$IFDEF SUPER_METHOD}
...
{$ENDIF}
    ...
    //Ocuco
    property jObject[const path: SOString]: ISuperObject read GetO write PutO;
    property jBoolean[const path: SOString]: boolean read GetB write PutB;
    property jInteger[const path: SOString]: SuperInt read GetI write PutI;
    property jDouble[const path: SOString]: Double read GetD write PutD;
    property jCurrency[const path: SOString]: Currency read GetC write PutC;
    property jString[const path: SOString]: SOString read GetS write PutS;
    property jArray[const path: SOString]: TSuperArray read GetA;
{$IFDEF SUPER_METHOD}
...
{$ENDIF}
    ...
    //Ocuco
    procedure AddJSONArray(ArrayName: string);
    procedure AddToJSONArray(ArrayName: string; JSONObj: ISuperObject); overload;
    procedure AddToJSONArray(ArrayName: string; AString: string); overload;
  end;
...
{$ENDIF}
  ...
  public
    ...
    // parser  ... owned!
    class function ParseString(AJSONString: string): ISuperObject; overload;
...
//Ocuco
function NewJSONObject(const AJSONString: SOString = '{}'): ISuperObject;
...
class function TSuperObject.ParseString(AJSONString: string): ISuperObject;
begin
  result := ParseString(pwidechar(widestring(AJSONString)), false, false);
end;
...
redo_char:
    ...
    tsEatws:
      begin
        if (SOIChar(v) < 256) and (AnsiChar(v) in spaces) then {nop} else begin
        (*if (v = '/') then
        begin
          tok.pb.Reset;
          tok.pb.Append(@v, 1);
...
procedure TSuperObject.AddJSONArray(ArrayName: string);
begin
  self.O[ArrayName] := SA([]);
end;

procedure TSuperObject.AddToJSONArray(ArrayName: string; JSONObj: ISuperObject);
begin
  if not Assigned(self.A[ArrayName]) then
    AddJSONArray(ArrayName);
  self.A[ArrayName].Add(JSONObj);
end;

procedure TSuperObject.AddToJSONArray(ArrayName: string; AString: string);
begin
  if not Assigned(self.A[ArrayName]) then
    AddJSONArray(ArrayName);
  self.A[ArrayName].PutS(self.A[ArrayName].Count, AString);
end;
